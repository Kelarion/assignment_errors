...
model {
  ...
  // likelihood
  for (n in 1:T) {
    nom = color_likelihood(y[n], cue[n]*C_u[n], (1-cue[n])*C_l[n], mu_u, mu_d_u,
    ...
    lp_swp[2] = (log_p_err[type[n]][2]
		 + color_likelihood(y[n], cue[n]*C_u[n], (1-cue[n])*C_l[n], mu_u, mu_d_u,
...
generated quantities{
  ...
  for (n in 1:T) {
    ...
    lp[1] = log_p[n][1] + color_likelihood(y[n], cue[n]*C_u[n], (1-cue[n])*C_l[n], mu_u, mu_d_u,
					   mu_l, mu_d_l, cue[n], vars, nu,
					   intercept_up, intercept_down);
    // spatial errors
    lp_swp[1] = (log_p_err[type[n]][1]
    ...
    lp_swp[3] = (log_p_err[type[n]][3]
		 + color_likelihood(y[n], cue[n]*C_u[n], (1-cue[n])*C_l[n], mu_u, mu_d_u, mu_l,
				    mu_d_l, cue[n], vars, nu,
				    intercept_up, intercept_down));
		 
    // swap errors (spatial and cue)                    
    lp[2] = log_p[n][2] + log_sum_exp(lp_swp);   
    lp[3] = log_p[n][3] + color_likelihood(y[n], cue[n]*C_u[n], (1-cue[n])*C_l[n], mu_u, mu_d_u, mu_l,
					   mu_d_l, cue[n], vars, nu,
					   intercept_up, intercept_down);
    ...
    if (trl_type==1)
    {
      err_hat[n] = color_rng(cue[n]*C_u[n], (1-cue[n])*C_l[n], mu_u, mu_d_u, mu_l, mu_d_l,
