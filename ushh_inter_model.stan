functions {
  real color_likelihood(vector resp, vector cu, vector cl,
    ...
    real out;
    out = (cue*(normal_lpdf(resp | mu_u*cu + mu_d_l*cl + intercept_up,
			    sqrt(vars))) 
	   + (1-cue)*(normal_lpdf(resp | mu_l*cl + mu_d_u*cu + intercept_down,
				  sqrt(vars))));
    return out;
  }
  ...
  vector color_rng(vector cu, vector cl, matrix mu_u, matrix mu_d_u,
    ...
    out = to_vector(normal_rng(cue*(mu_u*cu + mu_d_l*cl + intercept_up)
  ...
  for (n in 1:T) {
    ...
    if (trl_type==1)
    {
      err_hat[n] = color_rng(C_u[n], C_l[n], mu_u, mu_d_u, mu_l, mu_d_l,
			     cue[n], vars, intercept_up, intercept_down);
    }
    else if (trl_type==2)
    {
      if (swp_type==1)
      ...
      {
        err_hat[n] = color_rng(C_u[n], C_l[n], mu_u, mu_d_u, mu_l, mu_d_l,
			       1 - cue[n], vars, intercept_up, intercept_down);
      }
      else if (swp_type==3)
      {
        err_hat[n] = color_rng(C_u[n], C_l[n], mu_u, mu_d_u, mu_l, mu_d_l,
