...
parameters {
  ...
  simplex[2] p_err[is_joint ? 2:1]; // probability of "forgetting" uncued color
}

transformed parameters {
  vector[2] log_p_err[is_joint ? 2:1];
...
model {
  ...
  for (t in 1:size(p_err)){
    p_err[t] ~ dirichlet(rep_vector(1.5,2));
  }

  // likelihood
  for (n in 1:T) {
    lp[1] = log_p_err[type[n]][1] + color_likelihood(y[n], C_u[n], C_l[n], mu_u, mu_d_u,
         mu_l, mu_d_l, cue[n], vars, nu,
         intercept_up, intercept_down);

    // forgetting errors
    lp[2] = (log_p_err[type[n]][2]
...
generated quantities{
  ...
  for (n in 1:T) {
    ...
    trl_type = categorical_rng(p_err[type[n]]);
