...
parameters {
  ...
  simplex[4] p_err[is_joint ? 2:1]; // probability of a cue, spatial, or no error
}

transformed parameters {
  vector[4] log_p_err[is_joint ? 2:1];
...
model {
  ...
  for (t in 1:size(p_err)){
    p_err[t] ~ dirichlet(rep_vector(1.5,4));
  }

  // likelihood
  for (n in 1:T) {
    nom = color_likelihood(y[n], C_u[n], C_l[n], mu_u, mu_d_u,
			   mu_l, mu_d_l, cue[n], vars, nu,
			   intercept_up, intercept_down);
    lp[1] = log_p[n][1] + nom;

    // spatial errors
    lp_swp[1] = (log_p_err[type[n]][1]
		 + color_likelihood(y[n], C_l[n], C_u[n], mu_u, mu_d_u,
				    mu_l, mu_d_l, cue[n], vars, nu,
				    intercept_up, intercept_down));

    // cue errors
    lp_swp[2] = (log_p_err[type[n]][2]
		 + color_likelihood(y[n], C_u[n], C_l[n], mu_u, mu_d_u,
				    mu_l, mu_d_l, 1 - cue[n], vars, nu,
				    intercept_up, intercept_down));

    // forgetting errors
    lp_swp[3] = (log_p_err[type[n]][3]
     + color_likelihood(y[n], cue[n]*C_l[n], (1-cue[n])*C_u[n], mu_u, mu_d_u,
            mu_l, mu_d_l, cue[n], vars, nu,
            intercept_up, intercept_down));

    // no errors
    lp_swp[4] = log_p_err[type[n]][4] + nom;
...
generated quantities{
  ...
  for (n in 1:T) {
    ...
    lp[1] = log_p[n][1] + color_likelihood(y[n], C_u[n], C_l[n], mu_u, mu_d_u,
					   mu_l, mu_d_l, cue[n], vars, nu,
					   intercept_up, intercept_down);
    // spatial errors
    lp_swp[1] = (log_p_err[type[n]][1]
    ...
    lp_swp[3] = (log_p_err[type[n]][3]
     + color_likelihood(y[n], cue[n]*C_l[n], (1-cue[n])*C_u[n], mu_u, mu_d_u,
            ...
            intercept_up, intercept_down));
    // no errors
    lp_swp[4] = (log_p_err[type[n]][4]
		 + color_likelihood(y[n], C_u[n], C_l[n], mu_u, mu_d_u, mu_l,
				    mu_d_l, cue[n], vars, nu,
				    intercept_up, intercept_down));
		 
    // swap errors (spatial and cue)                    
    lp[2] = log_p[n][2] + log_sum_exp(lp_swp);   
    lp[3] = log_p[n][3] + color_likelihood(y[n], C_u[n], C_l[n], mu_u, mu_d_u, mu_l,
					   mu_d_l, cue[n], vars, nu,
					   intercept_up, intercept_down);
    ...
    if (trl_type==1)
    {
      err_hat[n] = color_rng(C_u[n], C_l[n], mu_u, mu_d_u, mu_l, mu_d_l,
    ...
    else if (trl_type==2)
    {
      if (swp_type==1)
      {
        err_hat[n] = color_rng(C_l[n], C_u[n], mu_u, mu_d_u, mu_l, mu_d_l,
			       cue[n], vars, nu, intercept_up, intercept_down);
      }
      else if (swp_type==2)
      {
      ...
      {
        err_hat[n] = color_rng(cue[n]*C_l[n], (1-cue[n])*C_u[n], mu_u, mu_d_u, mu_l, mu_d_l,
             cue[n], vars, nu, intercept_up, intercept_down);
      }
      else if (swp_type==4)
      {
