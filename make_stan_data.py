...
# these_models.append(['inter_t_precue', 'inter_tf_precue', 'null_t_precue'])
these_models.append(['inter_t_precue'])
# these_models.append(['ushh_inter', 'ushh_t_inter'])
these_models.append(['ushh_t_inter'])
# these_models.append(['noswap_delay2'])
	...
	for i_dst, dset_prm in enumerate(these_dsets):
		...
		for vals in list(itt.product(*var_v)):
			...
			for i, which_sess in enumerate(these_sess):
				...
				# print(yup[:,None].shape)
				# print(ylow.shape)
				c_u = color_weights(yup, num_bins).T
				c_l = color_weights(ylow, num_bins).T
